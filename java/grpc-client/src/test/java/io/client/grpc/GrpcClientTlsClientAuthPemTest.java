...
public class GrpcClientTlsClientAuthPemTest extends RpcClientTest {
  ...
  private static void createCA() throws Exception {
    ...
    final String createCert =
        String.format("openssl req -new -x509 -nodes -days 3600 -batch -config %s -key %s -out %s",
            caConfig, caKey, caCert);
    runProcess(createCert);
  }

  private static void createTrustStore() throws Exception {
    final String convertCaCert =
        String.format("openssl x509 -outform der -in %s -out %s", caCert, caCertDer);
  ...
  private static void createSignedCert(final String serial, final String name) throws Exception {
    ...
    final String signKey =
        String.format("openssl x509 -req -in %s -days 3600 -CA %s -CAkey %s -set_serial %s -out %s",
            req, caCert, caKey, serial, cert);
    runProcess(signKey);
  }
  ...
  private static void createKeyStore(final String name) throws Exception {
    ...
    final String key = certDirectory.getCanonicalPath() + File.separatorChar + name + "-key.pem";
    final String combo =
        certDirectory.getCanonicalPath() + File.separatorChar + name + "-combo.pem";
    ...
    final String combine = String.format("cat %s %s", cert, key);
    System.out.println(combine);
    new ProcessBuilder(combine.split(" "))
        .redirectOutput(new File(combo))
        .start()
        .waitFor();
  }
  ...
  private static void createClientConnection() throws Exception {
    final String keyStore =
        certDirectory.getCanonicalPath() + File.separatorChar + "client-combo.pem";

    final TlsOptions tlsOptions = new TlsOptions()
        .keyStorePath(keyStore)
        .keyStoreType("pem")
        // TODO we still don't support PEM here
