...
func createDiscoveryGateway(ctxWithSpan context.Context, hc discovery.HealthCheck, serv srvtopo.Server, cell string, retryCount int) Gateway {
	...
	for _, c := range strings.Split(*cellsToWatch, ",") {
		...
		ctw := discovery.NewCellTabletsWatcher(ctxWithSpan, topoServer, tr, c, *refreshInterval, *refreshKnownTablets, *topoReadConcurrency)
